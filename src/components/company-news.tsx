...
import { companyNews } from "@mock-data/news";
...
export default function CompanyNews() {
  ...
  const newsItems = useMemo(() => {
    return visibleItems?.map((item) => (
      <div
        key={item?.id}
        className="bg-pale-blue py-4 sm:py-[29px] px-4 flex flex-col gap-2 w-[258px] sm:w-auto rounded-sm sm:rounded-none"
      >
        <div className="flex justify-between items-center">
          <div className="rounded-sm p-1 bg-white text-moderate-pink font-semibold text-[9px]">
            {item?.subject}
          </div>
  ...
  return (
    <div className="flex flex-col text-black">
      <div className="sm:bg-pale-blue flex justify-between items-center px-4 sm:pt-4 pb-4 sm:pb-0">
        <h2 className="text-[18px] leading-[21.78px] font-bold">
          Company News
        </h2>
        {items.length > 2 && isMobile && (
          <button
            onClick={() => handleClick()}
      ...
      {items.length > 2 && !isMobile && (
        <div className="bg-pale-blue pb-4 px-2 flex flex-row-reverse">
          <button
            onClick={() => handleClick()}
